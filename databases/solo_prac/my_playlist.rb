...
def genre_exist_check(genre)
  ...
  @existing_genres = db.execute("SELECT id, mood FROM mood;")
  # calls .rassoc method on existing moods array, which returns nested array if second element 
  # in nested array contains value, to see if value is returned. If nil, mood does 
  # not exist and returns false
...
def create_item(db, name, link, filth_level, mood, genre)
  ...
  mood_id = mood_id_return(mood)
  genre_id = genre_id_return(genre)
  #inputs all variables to update the playlist table for new item
  db.execute("INSERT INTO playlist (name, link, filth_level, mood_id, genre_id) VALUES (?, ?)", [name, link, filth_level, mood_id, genre_id])
...
def recal(current_mood, current_genre, filth_limits)
...
p "Old folk and did-friendly? y or n"
answer = gets.chomp
if answer == "y"
	filth_level = 2
else filth_level = 1
